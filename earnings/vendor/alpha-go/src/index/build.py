...
def add_document_to_index(store: IndexStore, doc, markdown: str, config: dict, *, embedder) -> dict:
    ...
    try:
        ...
        # Replace-safe: drop any prior rows for this doc before re-inserting.
        old_ids = [r["chunk_id"] for r in
                   conn.execute("SELECT chunk_id FROM chunks WHERE doc_id=?", (doc.doc_id,)).fetchall()]
        for cid in old_ids:
