...
APPROVAL_MANIFEST = ALPHA_GO_ROOT / "vendor_divergences.json"
...
def _vendor_only_files() -> list[str]:
    ...
    unexpected: list[str] = []
    for pkg in VENDORED_PACKAGES:
        vendor_pkg = ALPHA_GO_ROOT / "src" / pkg
        parent_pkg = PARENT_ROOT / "src" / pkg
...
def drift() -> list[tuple[str, str]]:
    ...
    Declared alpha-go-only additions are intentional and are not drift. A missing
    managed file or undeclared vendor-only file is drift and blocks ``--sync``
    unless the destructive operation is explicitly forced.
    """
    ...
    for label, parent_file, alpha_file in _vendored_files():
        ...
        if not alpha_file.is_file():
            changed.append((label, "missing from alpha-go"))
            continue
