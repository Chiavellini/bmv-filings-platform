...
def extract_from_text_search(
    ...
    *,
    ...
    match_cache: dict[str, tuple[str, float] | None] = {}
    ...
    for idx, line in enumerate(lines):
        ...
        for logical in _logical_lines(lines, idx):
            ...
            fallback_score = 0.0
            if label in match_cache:
                cached = match_cache[label]
                if cached is None:
                    continue
            ...
            rank = _row_rank(label, context_lines)
...
def _row_rank(label: str, context_lines: list[str]) -> int:
