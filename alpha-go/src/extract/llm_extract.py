...
_SYSTEM = (
    ...
    "compute, or estimate. CURRENCY values must be returned in THOUSANDS of pesos "
    "(miles de pesos), matching the financial-statement tables: e.g. if the text "
    'says "$213.9 millones", return 213900; if a table row shows "589,053", return '
    "589053. Percentages and counts are returned as their plain number. For each "
...
def _cache_key(model: str, keys: list[str], text: str) -> str:
    text_hash = hashlib.sha256(text.encode("utf-8")).hexdigest()
    payload = f"{model}|{','.join(sorted(keys))}|{text_hash}"
...
def llm_extract(
    ...
    *,
    ...
    key = _cache_key(model, keys, text)
    ...
    if data is None:
        ...
        data = fn(model=model, system=_SYSTEM, user=user, schema=_build_schema(defs))
