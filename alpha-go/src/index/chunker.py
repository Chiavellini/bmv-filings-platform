...
def _heading_before(markdown: str, offset: int) -> str | None:
    ...
    for line_match in _HEADING_LINE.finditer(markdown):
...
def chunk_document(
    ...
    *,
    ...
    if not spans:
        return []
    # Resolve headings once. Calling _heading_before for every chunk rescanned
    # the entire prefix and became quadratic on multi-megabyte filings.
    heading_matches = list(_HEADING_LINE.finditer(markdown))
    heading_offsets = [match.start() for match in heading_matches]
    heading_labels = [match.group(1).strip() for match in heading_matches]
    ...
    while i < len(pieces):
        ...
        heading_index = bisect_right(heading_offsets, chunk_start) - 1
        chunks.append(Chunk(
