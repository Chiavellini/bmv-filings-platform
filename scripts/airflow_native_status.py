...
def inspect(environment_file: Path, *, runtime: bool = True) -> dict[str, Any]:
    ...
    mount = path_check("PDFS_ESTATE_MOUNT_ROOT", directory=True)
    ...
    if estate:
        identity = inspect_estate_environment(estate, values)
        checks["estate_identity"] = {
            "ok": identity.healthy,
            "detail": (
                identity.observed_estate_id
                if identity.healthy
                else "; ".join(identity.problems)
            ),
        }
    else:
        checks["estate_identity"] = {"ok": False, "detail": "estate missing"}
