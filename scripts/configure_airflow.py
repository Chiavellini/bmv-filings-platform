...
from estate_volume import inspect_estate_volume, read_estate_id
...
def render_environment(
    ...
    *,
    ...
    estate_mount_root: Path | None = None,
    ...
    selected_mount = (estate_mount_root or _nearest_mount_root(estate)).expanduser().resolve()
    selected_estate_id = estate_id or read_estate_id(estate)
    ...
    values = {
        ...
        "PDFS_ESTATE_MOUNT_ROOT": str(selected_mount),
        "PDFS_DOCUMENT_ESTATE": str(estate),
        "PDFS_ESTATE_ID": selected_estate_id,
...
def _nearest_mount_root(path: Path) -> Path:
    for candidate in (path, *path.parents):
        if candidate.is_mount():
            return candidate
    raise ValueError(f"could not determine mounted filesystem for {path}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "physical filesystem mount containing --estate; inferred from the "
            "mounted path when omitted"
        ),
    )
    parser.add_argument(
        "--airflow-home",
...
def main(argv: list[str] | None = None) -> int:
    ...
    mount_root = (
        args.estate_mount_root.expanduser().resolve()
        if args.estate_mount_root is not None
        else _nearest_mount_root(estate)
    )
    try:
        estate_id = read_estate_id(estate)
