...
def test_alpha_drain_is_applied_bounded_and_fail_closed(tmp_path: Path) -> None:
    command = build_command(
        ...
        environment=_alpha_environment(
            ...
            PDFS_AIRFLOW_PDF_PARSER_VERSION="pdf-v7",
            PDFS_AIRFLOW_XBRL_PROCESSOR_VERSION="xbrl-v4",
            PDFS_AIRFLOW_ALPHA_TIMEOUT_SECONDS="2400",
        ),
    )
    ...
    assert command[command.index("--parser-version") + 1] == "pdf-v7"
    assert command[command.index("--xbrl-processor-version") + 1] == "xbrl-v4"
    assert command[command.index("--alpha-timeout-seconds") + 1] == "2400"
...
def test_generated_environment_is_secret_bearing_and_sync_off(
    ...
    assert 'PDFS_AIRFLOW_ALPHA_TIMEOUT_SECONDS="1800"' in first
