...
| Document estate | `<mount-root>/bmv-estate-v1` | This project/user |
...
.airflow-venv/bin/python scripts/configure_airflow.py \
  --estate /absolute/path/on/external/disk/document-estate \
  --launchd-output-dir "$HOME/Library/LaunchAgents"
```
...
Both the production corpus projection and index belong on the external estate,
so a fresh clone can consume them without recreating machine-local state. A
portable layout example is:

```text
PDFS_AIRFLOW_ALPHA_ENABLED="false"
PDFS_ALPHA_ROOT="/absolute/path/to/repository/alpha-go"
PDFS_ALPHA_CORPUS="/absolute/path/to/estate/projections/alpha-go"
...
PDFS_ALPHA_PYTHON="/absolute/path/to/repository/alpha-go/.venv312/bin/python"
PDFS_ALPHA_TARGET_ID="portable-estate-v1"
```

Seed and certify `<estate>/projections/alpha-go` with a full reconcile, then
retain that exact path. Do not use the estate root itself as the corpus and do
not place the production corpus or index inside the Git checkout.
...
```bash
launchctl bootstrap "gui/$(id -u)" "$HOME/Library/LaunchAgents/com.bmv.airflow.api-server.plist"
launchctl bootstrap "gui/$(id -u)" "$HOME/Library/LaunchAgents/com.bmv.airflow.scheduler.plist"
launchctl bootstrap "gui/$(id -u)" "$HOME/Library/LaunchAgents/com.bmv.airflow.dag-processor.plist"
```
...
## Portable estate requirement

Do not point another computer at an ad-hoc copy of a live legacy catalog. A
complete portable export must:
...
`scripts/export_portable_estate.py` implements that boundary, and
`scripts/check_portable_estate.py` is the read-only destination-host gate. The
Estate USB must also pass mount-point, sentinel, and ID checks before any writer
task is enabled.
...
Destination-host service activation remains separate acceptance work and is
not implied by the checked-in templates.
