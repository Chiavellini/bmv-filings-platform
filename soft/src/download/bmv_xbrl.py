...
def download_ticker(
    ...
    *,
    ...
    wanted = [f for f in index if f.ticker.upper() == ticker.upper()]
    ...
    if not wanted:
        known = sorted({f.ticker for f in index})
        raise RuntimeError(
            f"No XBRL filings for ticker {ticker!r} in the BMV archive. "
            f"(Financial-sector issuers have no quarterly XBRL — use the IR downloader.) "
