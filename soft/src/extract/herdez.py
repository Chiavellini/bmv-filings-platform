        ...
        for pats in all_row_pats:
            if matches_row_start(candidate, pats):
                if _big_financial_nums(candidate):
                    return True
                break  # alias matched but no big num → narrative line (e.g. "Consolidated margin reached 40%")
        else:
...
def extract_segment_table(
    ...
    *,
    ...
    """Find a section header and extract labeled segment rows under it."""
    ...
    for i, line in enumerate(lines):
        ...
        if big_inline:
            result['consolidated'] = big_inline[0]
        else:
            ...
            if not matched_seg:
                ...
                # Try unlabeled number row (2025 two-line headers like "Utilidad de operación")
                    ...
                    if seg_stripped and (seg_stripped[0].isdigit() or seg_stripped[0] == '('):
                        ...
                        if big:
                            result['consolidated'] = big[0]

        break  # use first valid occurrence
