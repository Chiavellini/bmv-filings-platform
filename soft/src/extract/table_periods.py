...
# ---------------------------------------------------------------------------
# Column orientation for custom extractors that work on raw statement lines
# (not per-column header maps). These are the two primitives every custom
# extractor was re-rolling; see docs/EXTRACTION_ROADMAP.md (P0 #2).
# ---------------------------------------------------------------------------

def orient_by_growth(a: float, b: float, growth_pct: float | None) -> tuple[float, float]:
    """Return ``(current, prior)`` for two adjacent value columns by matching the
    printed period-over-period growth %.

    When a row gives two values ``a, b`` (in column order) plus a printed Δ%, the
    column order may be current-first or prior-first (it flips across issuers/eras).
    This picks the ordering whose implied growth best matches ``growth_pct``. With no
    growth signal (or a zero denominator) it leaves the order unchanged.
    """
    if growth_pct is None or a == 0 or b == 0:
        return a, b
    err_a = abs((a / b - 1.0) * 100.0 - growth_pct)   # a is current
    err_b = abs((b / a - 1.0) * 100.0 - growth_pct)   # b is current
    return (a, b) if err_a <= err_b else (b, a)


def current_column_in_header_line(
    header_line: str, target: PeriodKey, *, ncols: int = 2
) -> int | None:
    """Which column (0-based, among the first ``ncols`` period-bearing columns) is the
    current period, given a single header LINE whose period tokens appear in column order.

    Unlike :func:`select_value_for_period` (which takes a per-column header map and
    requires a quarter), this reads a whole header line — e.g. a statement header
    ``"ESTADO DE RESULTADOS 2T24 2T25"`` or a table header
    ``"(Miles de Dólares) 2025 2026 US$000 %"``. It matches quarter+year tags first,
    then falls back to plain 4-digit years (the year-labelled case, where the quarter is
    implied by the report period). Returns ``None`` when no column matches, so the caller
    owns the fallback (positional, or a value/growth tiebreak via :func:`orient_by_growth`).
    """
    norm = _norm(header_line)
    qtags = [(int(m.group(1)), _year4(m.group(2))) for m in _QTR_YEAR_RE.finditer(norm)]
    for idx, (q, y) in enumerate(qtags[:ncols]):
        if q == target.quarter and y == target.year:
            return idx
    years = [int(m.group(1)) for m in _YEAR_RE.finditer(norm)]
    for idx, y in enumerate(years[:ncols]):
        if y == target.year:
            return idx
    return None
