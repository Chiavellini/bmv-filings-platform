...
def pesos_per_unit_for_facts(cfg: dict | None, facts: dict | None) -> float:
    """Soft-compatible divisor for the legacy native coverage path.

    Root extraction calls :func:`fact_value_divisor_for` explicitly and receives
    the new denomination-aware behavior. Soft's coverage pipeline predates that
    contract and relies on this wrapper leaving ``miles_mxn`` configurations
    untouched; only a millions-config filer may activate its historical
    already-in-millions guard.
    """
