...
# Residual-category modifiers: a label like "Other Accounts Receivable" names a
# DIFFERENT line item than "Accounts Receivable" (otras cuentas por cobrar), so a
# contains/token-overlap hit against the unmodified alias is a false match
# (soriana 2025-1T: AR 6,285 "Other Accounts Receivable" vs true 1,056).
_RESIDUAL_LABEL_PREFIXES = frozenset({
    "other", "others", "otra", "otras", "otro", "otros",
})
...
def _alias_score(
    ...
    if (
        label_tokens
        and label_tokens[0] in _RESIDUAL_LABEL_PREFIXES
        and alias_tokens
        and alias_tokens[0] not in _RESIDUAL_LABEL_PREFIXES
    ):
        return 0.0, "residual_label"
    if (
        len(alias_tokens) == 1
    ...
    if len(alias_tokens) > 1:
        ...
        if (
            coverage >= 0.75
            and len(label_tokens) <= len(alias_tokens) + 2
            and _alias_tokens_in_label_order(label_tokens, alias_tokens)
        ):
            # Order matters: "payable and receivable accounts" (a cash-flow row)
            # must not satisfy the "accounts receivable" alias.
...
def _alias_tokens_in_label_order(
    label_tokens: tuple[str, ...],
    alias_tokens: tuple[str, ...],
) -> bool:
    """True if the alias tokens present in the label occur in alias order."""
    label_set = set(label_tokens)
    present = [token for token in alias_tokens if token in label_set]
    if not present:
        return False
    it = iter(label_tokens)
    return all(any(token == lab for lab in it) for token in present)
