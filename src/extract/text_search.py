...
def _alias_contains_match(
    ...
    label_head = norm_label.split()[0] if norm_label else ""
    best: tuple[str, float, int, int] | None = None
    for key, alias in alias_index:
        if len(alias) < 3:
            continue
        # "Other Accounts Receivable" is a residual line item, not "Accounts
        # Receivable" — a contains hit against the unmodified alias is false.
        if (label_head in _RESIDUAL_LABEL_PREFIXES
                and alias.split()[0] not in _RESIDUAL_LABEL_PREFIXES):
            continue
