...
def _page_words(page) -> list[dict]:
    """``page.extract_words`` with float-noise "rotated" chars normalized.

    Same pathology parse_pdf handles for layout text: pdfminer mis-flags
    horizontal chars as non-upright on skew noise, and word extraction then
    groups them as vertical runs. Healthy pages keep the exact page call.
    """
    from src.parse.parse_pdf import normalize_upright

    chars, changed = normalize_upright(page.chars)
    if changed:
        from pdfplumber.utils.text import extract_words as _extract_words
        return _extract_words(chars, use_text_flow=False, keep_blank_chars=False) or []
...
def resolve_table_scale(
    ...
    ``with_evidence=True`` returns ``(scale, evidence)`` where evidence is one of
    "config" | "env" | "xbrl" | "caption" | "default" — "default" means NO
    positive signal backed the scale, so monetary table values are unverified
    (the silent-1000× case for pre-XBRL / caption-less companies).
    """
    def _ret(scale: float, evidence: str):
        return (scale, evidence) if with_evidence else scale

    if cfg is not None and "table_scale" in cfg:
        return _ret(float(cfg["table_scale"]), "config")
    import os
    ...
    if doc is not None and getattr(doc, "scale_source", None) == "header":
        ...
        if any(abs(cand - c) < c * 1e-6 for c in candidates):
            return _ret(cand, "caption")

    return _ret(1.0, "default")
        ...
        for raw_lbl, cell_pairs in block.rows:
            ...
            if target is not None and block.header_by_col:
                ...
                if cur_str is not None:
                    ...
                    # A period header EXISTS but couldn't be aligned to the target
                    # (OCR-mangled / reordered / target year absent). The positional
                    # guess below may read the wrong column — tag it so confidence
                    # scoring and the verification worklist surface it instead of
                    # letting a reordered-column grab pass as a clean [table] read.
                    degraded = "[positional] "
            ...
            found[match.metric_key] = MetricRow(
                ...
                var_pct=None,
                unit=mdef.unit,
                source_line=f"[table] {degraded}{raw_lbl} {' '.join(cell_values[:3])}".strip()[:100],
            )
