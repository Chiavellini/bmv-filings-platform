...
def _contract_setting(
    environment: Mapping[str, str],
    name: str,
    *,
    default: str,
) -> str:
    """Validate a compact consumer contract/version token."""

    value = environment.get(name, default).strip()
    if not value or len(value) > 128 or "\n" in value or "\r" in value:
        raise AirflowTaskConfigurationError(
            f"{name} must be a non-empty single line (max 128 characters)"
        )
    return value
    ...
    *,
    ...
    if mode in _ACQUISITION_MODES:
        ...
        if mode == "drain":
            ...
            # ``run`` retains PR #1's root PDF-markdown, XBRL-facts, and
            # derivative-publication consumers. Alpha is an additional
            # subscription, not a replacement for those estate derivatives.
            command.extend(
                (
                    "--max-deliveries",
                    str(maximum),
                    ...
                    _contract_setting(
                        env,
                        "PDFS_AIRFLOW_PDF_PARSER_VERSION",
                        default="1",
                    ),
                    "--xbrl-processor-version",
                    _contract_setting(
                        env,
                        "PDFS_AIRFLOW_XBRL_PROCESSOR_VERSION",
                        default="1",
                    ),
                    "--enable-alpha-go",
                )
