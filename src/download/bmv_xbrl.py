...
# --- raw-cache gzip-at-rest -------------------------------------------------
# Raw XBRL instance JSONs are stored gzip-compressed (``<TICKER>_<PERIOD>.json.gz``,
# ~9× smaller) while the derived ``_facts.json`` / ``_mdna.html`` artifacts stay
# plaintext. Readers accept either form so a legacy plaintext cache still works.
RAW_EXTS = (".json.gz", ".json")


def _logical_stem(path: Path) -> str:
    """Strip a trailing ``.json.gz``/``.json`` then a trailing ``_facts``.

    ``Path("AC_2021-2T.json.gz").stem`` only drops ``.gz`` — using it to build the
    sibling ``_facts.json``/``_mdna.html`` names would yield ``AC_2021-2T.json_facts``.
    The ``_facts`` strip is the idempotency guard for re-runs fed an artifact path.
    """
    name = Path(path).name
    for ext in RAW_EXTS:
...
def _resolve_raw(path: Path) -> Path:
    """Return the on-disk raw file for a logical path, preferring the ``.gz`` form."""
    path = Path(path)
...
def _read_raw_text(path: Path) -> str:
    """Read a raw instance JSON as text, transparently un-gzipping a ``.json.gz``."""
    path = _resolve_raw(path)
...
def _download_filing_json(
    ...
    *,
    ...
    # Store gzip-compressed at rest (~9× smaller). ``.with_suffix`` would strip the
    # ``.gz`` off a ``*.json.gz`` dest, so build the temp name by appending.
    tmp = dest.with_name(f".{dest.name}.part-{uuid.uuid4().hex}")
    # A raw filing is content-addressed by the estate. Pin the gzip header time
    # so fetching identical JSON on different runs yields identical bytes/hash.
...
def download_ticker(
    ...
    *,
    ...
    if not wanted:
        ...
        raise RuntimeError(
            f"No XBRL filings for ticker {ticker!r} matching the requested kinds "
            "in the BMV archive. "
    ...
    for i, filing in enumerate(wanted, 1):
        dest = xbrl_dir / f"{_safe_name(filing.ticker)}_{filing.period}.json.gz"
        # Skip if we already have the filing in either form (a legacy plaintext
        # .json from before gzip-at-rest, or the .json.gz).
        existing = _resolve_raw(dest.with_name(dest.name[:-3]))
        if existing.exists():
            dest = existing
...
def extract_artifacts(json_path: Path, out_dir: Path | None = None) -> dict[str, Path]:
    ...
    # _logical_stem strips a .json.gz/.json suffix (Path.stem only drops .gz) and the
    # idempotency _facts suffix, so a re-run over an artifact regenerates the same names.
    stem = _logical_stem(json_path)
...
def load_mdna_text(json_path: Path) -> str | None:
    ...
    json_path = Path(json_path)
    mdna_path = json_path.with_name(_logical_stem(json_path) + "_mdna.html")
