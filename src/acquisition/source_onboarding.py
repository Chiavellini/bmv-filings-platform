...
def classify_link(url: str, *, title: str, source_page: str) -> LinkCandidate:
    ...
    # Prefer the period named by the URL's own filename over the one in the link
    # text. When the two disagree the file wins: CEMEX publishes an anchor
    # reading "1Q26 Report" that points at `.../2q26-report-eng`, and labelling
    # that candidate 2026-1T made the downloaded PDF fail its own first-page
    # confirmation check, rejecting a perfectly good source. Link text remains
    # the fallback for URLs that carry no period at all (q4inc attendee IDs).
    stem = unquote(normalized).rsplit("/", 1)[-1]
    period = infer_period_label(stem) or infer_period_label(signal)
